...
try:
    ...
    raise ImportError("The `redisvl` package is not installed. Please install it via `uv pip install redisvl`.")
...
class Redis(VectorDb):
    ...
    def _create_schema(self):
        ...
        schema = {
            "version": "0.1.0",
            "index": {"name": self.index_name, "prefix": self.index_name, "key_separator": ":", "storage_type": "hash"},
            "fields": [
                {"name": "data", "type": "text"},
                {
                    "name": "data_embedding",
    ...
    def search(self, query: str, limit: int = 5, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
        ...
        query = VectorQuery(
            vector=query_embedding, vector_field_name="data_embedding", return_fields=["data"], num_results=limit
        )
