...
from agno.document.reader.single_csv_reader import SingleCSVReader
from agno.knowledge.agent import AgentKnowledge


class SingleCSVKnowledgeBase(AgentKnowledge):
    path: Union[str, Path]
    exclude_files: List[str] = Field(default_factory=list)
    reader: SingleCSVReader = SingleCSVReader()
    ...
    def document_lists(self) -> Iterator[List[Document]]:
        ...
        if _csv_path.exists() and _csv_path.is_dir():
            raise IsADirectoryError(f"Expected a file, but found a directory: {_csv_path}")
        elif _csv_path.exists() and _csv_path.is_file():
            if not (_csv_path.suffix == ".csv"):
                raise ValueError(f"Invalid file type: {_csv_path.suffix}. Expected a .csv file.")
