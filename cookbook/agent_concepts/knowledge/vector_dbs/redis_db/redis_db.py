...
documents = create_document("/home/yashm94/Projects/Redis/RedisVL/dataset/questions.csv")
...
results = vector_db.search(query=query, limit=3) #Searching for similar vector via query vector
for result in results:        #printing top k values where k is the limit set in `search` above
